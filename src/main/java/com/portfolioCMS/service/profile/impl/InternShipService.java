...
import com.portfolioCMS.repository.profile.IntershipRepo;
...
public class InternShipService implements IInternshipService {
    private  final IntershipRepo repo;

    public InternShipService(IntershipRepo repo) {
        super();
        this.repo = repo;
    }

    @Override
    public ResponseDTO create(@Valid InternshipDTO req) {
        if (req == null) {
        ...
        try {
            ...
            Internship saved = repo.save(entity);
    ...
    public ResponseDTO getAll() {
        try {
            List<InternshipDTO> dtoList = repo.findAll()
                    .stream()
                    .map(this::mapToDTO)
                    .toList();
    ...
    public ResponseDTO getById(int id) {
        ...
        try {
            Internship data = repo.findById((long) id)
                    .orElseThrow(() -> new RuntimeException("Data not found with ID: " + id));

            InternshipDTO dto = mapToDTO(data);
    ...
    @Override
    public ResponseDTO update(Long id, InternshipDTO req) {
        if (id == null || id <= 0) {
        ...
        Internship data = repo.findById(id)
                .orElseThrow(() -> new  RuntimeException("Data not found with ID: " + id));

        updateFields(data, req);

        Internship updated = repo.save(data);

        return new ResponseDTO(true, "Updated successfully", mapToDTO(updated));
    }

    private void updateFields(Internship data, InternshipDTO req) {

        Optional.ofNullable(req.getCompanyName()).ifPresent(data::setCompanyName);
        Optional.ofNullable(req.getRole()).ifPresent(data::setRole);
    ...
    private InternshipDTO mapToDTO(Internship data) {
        return  new InternshipDTO(
                data.getId(),data.getCompanyName(),data.getRole(),data.getStartDate(),data.getEndDate(), data.getDescription()
        );
